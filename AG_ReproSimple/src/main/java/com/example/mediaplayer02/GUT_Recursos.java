...
public class GUT_Recursos {
    
    
    public static Uri getURIRecursoFromNombreRecurso(Context context, String tiporecurso, String nombrerecurso){
        Resources resources = context.getResources();
        int idRecurso = resources.getIdentifier(nombrerecurso, tiporecurso, context.getPackageName());
        return Uri.parse("android.resource://" + context.getPackageName() + "/" + idRecurso);
    }
     ...
     *
     * @param context objeto de la instancia de Contexto que invoca el método
     * @param nombreRecurso nombre del fichero imagen que se intenta obtener
     * @return un bitmap con el fichero obtenido, o null si hay error o no existe el fichero
     */

    public  static  Drawable getDrawableFromAsset(final Context context, String nombreRecurso) {
        Drawable res = null;
        try {
            // get input stream
            InputStream ims = context.getAssets().open(nombreRecurso);
            // load image as Drawable
            res = Drawable.createFromStream(ims, null);
        } catch (IOException ex) {
            ex.printStackTrace();
    ...
    public static  Uri getURIFromAssetName(final Context context, String nombreRecurso) {
        return Uri.parse("file:///android_asset/"+nombreRecurso);      
    }
